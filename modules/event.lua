...
function EVENT.export(path)
	local nobeat_str = "{\"version\":\"0.4\",\"eventsData\":[{\"track\":\"NoBeat\",\"events\":["
	local nobeat_tab = MEM.event_data.table.eventsData[nobeat_track_index].events
	...
	local signal_tab = MEM.event_data.table.eventsData[event_track_index].events
...
local function open_edit_box_nobeat()
	...
	if edit_index then
		...
		edit_end = tonumber(MEM.event_data.table.eventsData[nobeat_track_index].events[edit_index].endSample) or edit_start
	else
...
local function open_edit_box_event()
	...
	if edit_index then
		...
		edit_end = tonumber(MEM.event_data.table.eventsData[event_track_index].events[edit_index].endSample) or edit_start
